...
module "ansible" {
  ...
  subnet_id              = data.aws_subnet.selected.id # AWS default VPC subnet [ansible is in aws default VPC ]
  user_data = file("ec2-provision.sh") # Configuring servers via provisioning 
  tags = merge(
    var.common_tags,
    {
        Component = "ansible"
